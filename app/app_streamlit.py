...
if st.session_state.atualizar:
    ...
    if ('Argenprop' in st.session_state.base_busca) and ('Zonaprop' in st.session_state.base_busca):
        ...
        df_final = (
            ...
            .pipe(
                lambda df: df.loc[
                    (df.distancia_unr.between(0, st.session_state.distancia_unr if st.session_state.distancia_unr > 0 else 60))
                    & (df.distancia_hospital_provincial.between(0, (st.session_state.distancia_provincial if st.session_state.distancia_provincial > 0 else 60)))
                    & (df.distancia_hospital_ninos.between(0, (st.session_state.distancia_ninos if st.session_state.distancia_ninos > 0 else 60)))
                    & (df.distancia_hospital_carrasco.between(0, (st.session_state.distancia_carrasco if st.session_state.distancia_carrasco > 0 else 60)))
                    & (df.distancia_hospital_baigorria.between(0, (st.session_state.distancia_baigorria if st.session_state.distancia_provincial > 0 else 60)))
                ]
            )
